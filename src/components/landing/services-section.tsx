            ...
            <div className="max-w-6xl mx-auto">
                <FadeIn className="mb-14">
                    ...
                    <h2 className="font-heading font-bold text-4xl sm:text-5xl text-northpeak-text mb-3 tracking-tight">
                        <GradientText colors={["#ffffff", "#00e5a0", "#ffffff"]} animationSpeed={6}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {services.map((svc) => (
                        <SpotlightCard key={svc.num} spotlightColor={svc.featured ? "rgba(0, 229, 160, 0.15)" : "rgba(255, 255, 255, 0.08)"} className={`rounded-2xl border ${svc.border} bg-northpeak-card ${svc.featured ? "ring-1 ring-northpeak-green/30 bg-northpeak-green/5" : ""}`}>
                            <TiltCard intensity={8} className="flex flex-col gap-5 p-5">
                                {svc.featured && (
